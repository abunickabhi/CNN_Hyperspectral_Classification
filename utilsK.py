...
def percent_stretch(b, perc=0.01):
    ...
    if len(band) == 0: # all NaN's
        ...
        band.sort()
        min_ = float(band[perc*len(band)]) # why float???
        max_ = float(band[(1-perc)*len(band)])
...
def percent_percent_stretch(b, percmin=0.01, percmax=0.99):
    ...
    if len(band) == 0: # all NaN's
        ...
        band.sort()
        min_ = float(band[percmin*len(band)]) # why float???
        max_ = float(band[percmax*len(band)])
        return (255.99*(b-min_)/(max_-min_)).clip(0, 255).astype('u1'), min_, max_
